...
import scrapers, { ScraperId, getAnimeScraper } from '../sources';
import { MediaType } from '../types/anilist';
import { Episode, SourceAnime } from '../types/data';
...
export const scrapeNewAnime = async (scraperId: ScraperId) => {
  try {
    ...
    console.log(`run scrapeNewAnime `, sourceAnime);
    const sourceMediaIds = sourceAnime.map((data) => data.sourceMediaId);

    const { data: connections, error } = await supabase
      .from('anime_source')
      .select('*, anime_episodes:anime_episodes(*)')
...
const scrapeAllNewAnime = async () => {
  try {
    for (const scraperId of Object.keys(scrapers.anime)) {
      await scrapeNewAnime(scraperId as ScraperId);
    }
  } catch (err) {
    logger.error(err.message);
  }
